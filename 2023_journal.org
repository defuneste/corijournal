...
- not eally aggregate, just some count and max see it with drew
...
** FCC
- [ ] structure table for Drew
- [ ] I need boolean with cdt and max with cdt
...
** FCC
- [ ] terminate the ~sql~ that convert raw into structured
- [ ]move S3 to RDS
